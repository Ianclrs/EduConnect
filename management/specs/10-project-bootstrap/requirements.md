...
| FR10 | Migrations do EF Core DEVEM ser aplicadas automaticamente na inicialização em modo Development via `db.Database.MigrateAsync()`, com falha capturada e logada sem interromper a aplicação | AC5 |
...
| E5 | Migration do EF Core falha (ex.: schema incompatível em BD inexistente) | Auto-migration na inicialização captura exceção e loga warning via Serilog. API continua executando normalmente — `/health/db` retorna 503 |
