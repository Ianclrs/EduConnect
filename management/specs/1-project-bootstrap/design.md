# Spec 1: Design — Project Bootstrap & Infrastructure
...
|---|---|---|
| Use `AppDbContext` directly in controller (no service layer) | Bootstrap has zero business logic. Service layer would be a pass-through with no value | Service layer (added in Spec 4+) |
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: ${DB_PASSWORD:-edugestor_dev}
    ports:
      - "5432:5432"
    volumes:
  ...
  api:
    ...
    environment:
      ...
      - ConnectionStrings__Default=Host=postgres;Database=edugestor;Username=edugestor;Password=${DB_PASSWORD:-edugestor_dev}
    ports:
      - "5000:8080"
    depends_on:
      postgres:
