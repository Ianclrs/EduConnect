...
| `TenantMiddleware` | ASP.NET Core middleware extracting tenant from JWT | `src/EduGestor.Api/Middleware/TenantMiddleware.cs` | `ITenantContext`, `HttpContext` |
...
| `TenantSeeder` | Seeds default tenant on first run | `src/EduGestor.Infrastructure/Data/Seeders/TenantSeeder.cs` | `AppDbContext`, `IWebHostEnvironment` |
